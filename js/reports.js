...
<pre>  {
  ...
  },
  {
    ...
    "pageNumberIdentified": "8",
    "articleColumnTitle": "E.A.W. Branch Activities - Glasgow and District Branch",
    "fullStringName": "Mr. A. P. McKim",
    "honorificOrTitle": "Mr.",
    "nameStandardized": "A. P. McKim",
    "jobTitleOrRole": "Lecturer",
    "associatedOrganization": "unknown",
    "predictedGenderFromContext": "male",
    "briefContext": "Gave useful and instructive lectures on fuses, plugs and sockets, and simple electrical practices",
    "relationToText": "referenced/quoted",
    "confidenceLevel": "high"
  },</pre>

<p>It is interesting to see that while both models extracted information relating to the same first person, they extracted a different sequence and names of people. The case of "Bailie W. B. Smith" is especially interesting. While Gemini identified this person, Claude Sonnet completely missed it. It also looks as if Gemini provided the output in order of appearance in the text, while Claude Sonnet 4.5 provided a random order. [Although order is not essential, for immediate human QA, it is a more convenient output style.] Because of these reasons, I decided to continue the tests with Gemini instead of Claude.</p>

<p>While the JSON output schema was useful, the initial review showed that I would look for additional information in the output too. As a result, I decided to trial the extraction of the following bits of information:</p>
<ul>
  <li>Page number (LLM captured)</li>
  <li>Page transcription (LLM captured)</li>
  <li>Exact name of person as it appears (LLM captured)</li>
  <li>Honors/Titles of the person (LLM captured)</li>
  <li>Standardised name for indexing (LLM captured)</li>
  <li>Person's job/profession mentions (LLM captured)</li>
  <li>Person's Associated organisation (LLM captured)</li>
  <li>Person's gender (LLM captured)</li>
  <li>10-word context about person (LLM captured)</li>
